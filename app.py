...
def register():
    if request.method == "POST":
        ...
        token = secrets.token_urlsafe(32)
        expires_at = generate_verification_expiry()

        try:
            if user:
                if user[5]:
                    return render_template("register.html", error="This email is already registered. Please log in.")
                database.refresh_verification(user[0], token, expires_at)
            else:
                database.create_pending_user(first, last, email, token, expires_at)

            verification_url = build_public_url("verify_email", token=token)
            delivery_mode = send_verification_email(email, first, verification_url)
            return render_template(
                "check_email.html",
                email=email,
                delivery_mode=delivery_mode,
                verification_url=verification_url if delivery_mode == "console" else None,
            )
            ...
            return render_template(
                "register.html",
                error="We could not send the confirmation email right now. Please try again.",
            )
...
def login():
    if request.method == "POST":
        ...
        if user and not user[5]:
            return render_template("login.html", error="Please verify your email and set your password before logging in.")
